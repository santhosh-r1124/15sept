    ...
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col px-4 py-6">
      <div className="flex items-baseline justify-between gap-2">
        ...
        <button
          ...
          disabled={unread === 0}
          className="text-xs font-medium text-blue-700 hover:underline disabled:opacity-40 disabled:no-underline"
        >
          Mark all read
        </button>
      </div>
              ...
              <>
                <div className="flex items-baseline justify-between gap-2">
                  <span className={`text-sm ${n.read ? 'text-slate-700' : 'font-semibold'}`}>
                    {!n.read && (
                      <span
                        className="mr-2 inline-block h-2 w-2 rounded-full bg-blue-600"
                        aria-label="Unread"
                      />
                    )}
                    {n.title}
